"""
    ...
    This build upon the last assignment with CSV data instead of hard-coded data in dictionaries. I used the CSV
    Python module to read in data as a list and parse through that list to write to the file, mod6_output.txt.
"""

# Import All (*) from Other Module
from Eric_Zorn_mod6_classes_week6 import Investor, StockData, Bonds, buildTable
import csv

def code_run():
    # Assigns Investor Class Inheritance for the User Bob Smith and Steve Jobs
    user_bob = Investor("Bob", "Smith", "123 Main Street", "212-453-9046")
    ...
    with open('Lesson6_Data_Stocks.csv', 'r') as rf:
        ...
        appl = StockData("appl", 12, 178.12, 185.34)
        ...
        steve_stocks_dict["appl"] = {
            "symbol": appl.symbol,
            "shares": appl.shares_owned,
            "purchase": appl.purchase_price,
            "current": appl.current_price,
        ...
        with open('mod6_output.txt', 'w') as wf:
            wf.write("Stock Information for {0}.\nHis phone number is {1} and his address is {2}.\n"
                          "--------------------------\n".format(user_bob.full_name, user_bob.phone_number,
                                                              user_bob.address))

            for title in titles[0]:
                wf.write("{0:5} | ".format(title))

            # Build Stock Table with loop over each value in the stocks_dict dictionary
            wf.write("\n")
            try:
            ...
            # Error Handling for writing dictionary values of stock information for Bob Smith user
            except:
                ValueError("Value is not in the stocks_dict dictionary")
            ...
            # Second Title Declaration
            for title in titles[0]:
                wf.write("{0:5} | ".format(title))

            # Apple Dictionary
            for val in steve_stocks_dict.values():
                wf.write("\n{0:5} |  {1:9} |  ${2:12} |  ${3:11} |  {4:12} |  ${5:14} |  {6:4} | ".format(
                    val["symbol"],
                    val["shares"],
                    val["purchase"],
                    val["current"],
    ...
    with open('Lesson6_Data_Bonds.csv', 'r') as bond_rf:
        ...
        # Appending to the bond_info list and simple error handling for values
        for line in bond_reader:
            if line != "" or line != 0 or line != None:
                bond_info.append(line)
            else:
                raise ValueError("The value cannot properly be appended to the bond info list")
        ...
        with open("mod6_output.txt", "a") as bond_wf:
            ...
            bond_wf.write("Bonds Information for {0}.\nHis phone number is {1} and his address is {2}.\n"
                    "--------------------------".format(user_bob.full_name, user_bob.phone_number, user_bob.address))


            # Build Bonds Table Method Call fo Bob Smith's Bonds
            bob_smith_bond.buildTable(
                bond_wf,
                bob_smith_bond.id,
